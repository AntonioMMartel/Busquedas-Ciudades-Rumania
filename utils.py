...
#Clase de ramificación y acotación.
class RYA(Queue):
    """A First-In-First-Out Queue."""
    ...
    #El extend reordena de menor a mayor coste
    def extend(self, items):
        def OrdenaCostes(n):
            return n.path_cost
        self.A.extend(items)
        #print("Vector a insertar: ", items)
        self.A.sort(key=OrdenaCostes)
        #print("Vector lista abierta: ", self.A)

    #El pop escoge el estado con menor coste. Con el código que hay pilla el primero.
    #En el graph search cuando hacemos pop apuntamos a este nodo. Por ello, tiene que ser el menor de todos.
    #en realidad, con que el extend me ponga el nodo más eficiente al principip esto no hay que cambiarlo.
    def pop(self):
        #Apuntamos al primer estado de la lista abierta
        e = self.A[self.start]
        #Aumentamos contador
        self.start += 1
        #Esto de abajo no es muy relevante.
        if self.start > 5 and self.start > len(self.A) / 2:
            self.A = self.A[self.start:]
            self.start = 0
        return e
...
class RYA_H(Queue):
    ...
    #El extend reordena de mayor a menor coste de camino
    def extend(self, items):
        def OrdenaCostes(n):
            #print("Nodo:", n, "Coste:", n.path_cost + self.problem.h(n))
