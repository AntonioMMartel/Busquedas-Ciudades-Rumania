...
def graph_search(problem, fringe):
    ...
    nodosVisitados = 0
    while fringe:
        #Cuando hacemos pop, quitamos ese nodo de la lista abierta y apuntamos a él.
        node = fringe.pop()
        nodosVisitados = nodosVisitados + 1
        if problem.goal_test(node.state):
            return [node, nodosVisitados]
        if node.state not in closed:
            closed[node.state] = True
            #Justo aqui se implementa el algoritmo de búsqueda (BFS o DFS)
            #Importante saber que una cola implementa el BFS y una pila el DFS
            fringe.extend(node.expand(problem))
    return [None, nodosVisitados]

def ramificacion_y_acotacion_search(problem):
...
def graph_search_informed(problem, fringe):
    closed = {}
    fringe.append(Node(problem.initial))
    # Se toma en cuenta la heuristica.
    fringe.setProblem(problem)
    #Contador de nodos visitados.
    nodosVisitados = 0
    while fringe:
        #Cuando hacemos pop, quitamos ese nodo de la lista abierta y apuntamos a él.
        ...
        if node.state not in closed:
            closed[node.state] = True
            #le paso el problema para que pueda resolver la heurística.
            fringe.extend(node.expand(problem))
    return [None, nodosVisitados]

def ramificacion_y_acotacion_informada_search(problem):
    return graph_search_informed(problem, RYA_H())
